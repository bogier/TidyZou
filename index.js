...
function getTop3Week(){
  return (children || [])
    .map((c, idx) => ({
      idx,
      name: (c?.settings?.childName || "Enfant").trim(),
      avatar: c?.settings?.avatar || "img/default.png",
      score: computeWeekForChildIdx(idx)
    }))
    .filter(p => p.score > 0)   // ✅ IMPORTANT : exclut les semaines non renseignées
    .sort((a,b) => b.score - a.score)
    .slice(0,3);
}


function renderPodiumAvatars(prefix, top3){
  // prefix = "podium-week" → ids: podium-week-1/2/3
  const slots = [
    document.getElementById(`${prefix}-1`), // 1er (milieu)
    document.getElementById(`${prefix}-2`), // 2e (gauche)
    document.getElementById(`${prefix}-3`)  // 3e (droite)
  ];
...
function renderHome(){
  ...
  if(!hasChildren){
	...
	const podiumSection = document.getElementById("homePodiums");
    if (podiumSection) podiumSection.style.display = "none";

    return;
  }
   // === Podium accueil (semaine) ===
  const podiumSection = document.getElementById("homePodiums");
  if (podiumSection) podiumSection.style.display = "block";

  const topWeek = getTop3Week();
  renderPodiumAvatars("podium-week", topWeek);
...
function bindMaintenanceButtons() {
  ...
  if (resetBtn) resetBtn.onclick = resetAllChildren;

  const purgeBtn = document.getElementById("btnPurgeAll");
  if (purgeBtn) purgeBtn.onclick = purgeAll;
}
      ...
      reg.addEventListener('updatefound', () => {
        ...
        newWorker.addEventListener('statechange', () => {
          if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
            // Le nouveau SW est déjà actif grâce à self.skipWaiting()
            // On affiche juste l'info à l'utilisateur
            showUpdateBanner();
          }
        });
      });
